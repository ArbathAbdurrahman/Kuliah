...
int main() {
    omp_set_num_threads(5); // Set thread
    int loop = 100; // Jumlah Loop
    double start_time = omp_get_wtime(); // Mulai pengukuran waktu

    #pragma omp parallel // Thread akan dibuat untuk mengeksekusi blok kode secara paralel
    {
        #pragma omp single // Hanya satu thread yang mengeksekusi blok kode
        {
            #pragma omp task // Tugas dibuat untuk dijalankan secara paralel oleh thread yang tersedia
            {
